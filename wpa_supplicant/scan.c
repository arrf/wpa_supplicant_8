				...
				struct wpa_driver_scan_params *params)
{
	...
	int frequencyToProbe[20];
	int frequency;
	if(fconfig!=NULL)
	{
		int i=0;
		while(fscanf(fconfig, "%d", &frequency)>0)
		{
			frequencyToProbe[i] = frequency;
			i++;
		}
		if(frequencyToProbe[0]!=0)
		{
			params->freqs = os_zalloc((i+1) * sizeof(int));
			if (params->freqs)
			{
				int j=0;
				for(j=0; j<i; j++)
					params->freqs[j] = frequencyToProbe[j];
			}
		}
		fclose(fconfig);
	}
